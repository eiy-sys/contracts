...
contract FallbackManager is SelfAuthorized {
    ...
    fallback () //changed from function to fallback 
        external payable
    
    {
        ...
        if (handler != address(0)) {
            ...
            assembly {
                ...
                let success := call(gas(), handler, 0, 0, calldatasize(), 0, 0) //changed from gas to gas()
...
contract GnosisSafe
        ...
        returns (uint256 payment)
    {
        ...
        address payable receiver = payable(refundReceiver) == payable(address(0)) ? payable(tx.origin) : payable(refundReceiver);
    ...
    */
        ...
        returns (bytes32)
    {
        ...
        return keccak256(
            abi.encodePacked(bytes1(0x19), bytes1(0x01), domainSeparator, safeMessageHash) //changed from byte to bytes1
        );
    }
